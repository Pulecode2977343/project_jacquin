    ...
    switch (roleId) {
        ...
        case 2: // Docente
            ...
            if (modTeacherMain) {
                ...
                if (teacherAvatar) {
                    teacherAvatar.src = resolvePath(sessionUser.avatar_url || 'assets/images/avatars/default_avatar.svg');
                }
            }
        ...
        case 4: // Aspirante (Legacy)
            ...
            if (modStudentMain) {
                ...
                if (avatarEl) {
                    avatarEl.src = resolvePath(sessionUser.avatar_url || 'assets/images/avatars/default_avatar.svg');
                }
            }
    ...
    if (userWidget) {
        userWidget.innerHTML = `
            <div style="display:flex; align-items:center; gap:10px; cursor:pointer;" onclick="window.openMyProfile()">
                ...
                <img src="${resolvePath(sessionUser.avatar_url || 'assets/images/avatars/default_avatar.svg')}" 
                     id="dashboard-avatar"
                            ...
                            ${students.map((s, i) => `
                                <div style="display:flex; align-items:center; gap:12px; background:rgba(255,255,255,0.05); padding:10px 15px; border-radius:10px; border:1px solid rgba(255,255,255,0.05);">
                                    <div style="width: 35px; height: 35px; border-radius: 50%; overflow: hidden; border: 1px solid rgba(255,255,255,0.2);">
                                        <img src="${s.avatar_url ? (s.avatar_url.startsWith('http') ? s.avatar_url : ApiService.BASE_URL + s.avatar_url) : 'assets/images/avatars/default_avatar.svg'}" style="width:100%; height:100%; object-fit:cover;">
                                    </div>
                                    <div style="flex:1;">
    ...
    if (res.success) {
        ...
        container.innerHTML = teachers.map(t => `
            <div class="card-glass" style="text-align:center;">
                <img src="${t.avatar_url ? (t.avatar_url.startsWith('http') ? t.avatar_url : ApiService.BASE_URL + t.avatar_url) : 'assets/images/avatars/default_avatar.svg'}" style="width:80px; height:80px; border-radius:50%; object-fit:cover; margin-bottom:10px;">
