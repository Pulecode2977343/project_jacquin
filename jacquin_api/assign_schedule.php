...
try {
    ...
    if (!isset($data->course_id) || !isset($data->day) || !isset($data->start_time) || !isset($data->end_time)) {
        throw new Exception("Datos incompletos para el horario. (Recibido: " . print_r($data, true) . ")");
    }
    ...
    // Insert Schedule - USING CORRECT COLUMNS: time_start, time_end, id_course
    $sql = "INSERT INTO schedules (id_course, day, time_start, time_end, teacher_id) 
            VALUES (:cid, :day, :start, :end, :tid)";

    $stmt = $pdo->prepare($sql);
    $stmt->execute([
        ':cid' => $data->course_id, // Input is called course_id, column is id_course
        ':day' => $data->day,
